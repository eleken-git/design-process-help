...
[`docs/3d/index.html`](docs/3d/index.html) — плеєр на Three.js з перемикачем епізодів (кнопка «Епізоди»), розділами, паузою, перемоткою і музикою з кнопкою вимкнення. Керується як YouTube: клік по відео — пауза/плей, подвійний клік або `F` — на весь екран, `Space`/`K` — пауза, `←`/`→` — ±5 с, `J`/`L` — ±10 с, `M` — музика, `0`–`9` — стрибок по епізоду; панель ховається, коли миша не рухається. Кожен епізод має свій трек у стилі cosmic relaxation з фортепіано.
