...
| 3D-розбір конфлікту + відео | [`docs/conflict-3d/index.html`](docs/conflict-3d/index.html) — сцена з паузою і перемоткою |
...
## 7a. Конфлікт у 3D: анімація і відео

[`docs/conflict-3d/index.html`](docs/conflict-3d/index.html) — інтерактивна 3D-сцена на Three.js: дві гілки, той самий рядок, конфлікт і три способи його розв'язати. Пробіл ставить на паузу, стрілки перемотують, чипи внизу стрибають по розділах.

Дев'ять розділів: одна точка старту → той самий файл → обоє змінюють рядок 13 → Аня зливає першою → `git merge main` і CONFLICT → що git написав усередині файлу → варіанти A / B / C → прибрали маркери → профілактика.

Ту саму сцену відрендерено у відео 1:46 з текстом і ембіент-музикою. Наступні теми — у [`ANIMATIONS.md`](ANIMATIONS.md), обирай номер.
