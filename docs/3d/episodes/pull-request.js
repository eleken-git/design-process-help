...
window.EPISODES.push({
  ...
  build(ctx) {
    ...
    const BEATS = [
      ...
      { id: 'branch', t0: 6.5, t1: 16, num: '01', ttl: 'Гілка, два коміти, push',
    ...
    function prCard(o) {
      ...
      // заголовок
      g.fillStyle = HEX(C.text); g.font = '600 46px ' + SANS; g.fillText('dashboard: картка відтоку', 44, 70);
      g.fillStyle = HEX(C.muted); g.font = '400 40px ' + SANS; g.fillText('#21', 44 + g.measureText('dashboard: картка відтоку ').width + 14, 72);
      ...
      // рядки статусу
      const rows = [
        ['Рев’юер', o.state === 'draft' ? '— ще не запрошено' : (o.state === 'open' ? 'Богдан · чекаємо' : (o.state === 'changes' ? 'Богдан · Changes requested' : 'Богдан · Approved ✓')),
      ...
      g.fillText(o.state === 'merged' ? 'один коміт у main: "dashboard: картка відтоку (#21)"' : (ready ? 'усі умови виконані' : (o.state === 'draft' ? 'чернетка: злиття заблоковано' : 'потрібні Approve і зелений check')), bx + bw + 30, by + bh / 2);
      return T(el);
    }
    /** Коментар рев’юера до рядка */
    function commentCard(resolved) {
      const W = 1280, H = 800, el = cv(W, H), g = el.getContext('2d');
      const col = resolved ? HEX(C.green) : HEX(C.orange);
      g.fillStyle = HEX(C.surface); rr(g, 8, 8, W - 16, H - 16, 26); g.fill();
      g.strokeStyle = col; g.lineWidth = 6; rr(g, 8, 8, W - 16, H - 16, 26); g.stroke();
      g.textBaseline = 'middle';
      g.fillStyle = HEX(C.orange); g.beginPath(); g.arc(80, 76, 34, 0, Math.PI * 2); g.fill();
      g.fillStyle = HEX(C.bg); g.font = '700 34px ' + SANS; g.textAlign = 'center'; g.fillText('Б', 80, 78); g.textAlign = 'left';
      g.fillStyle = HEX(C.text); g.font = '600 38px ' + SANS; g.fillText('Богдан', 136, 60);
      g.fillStyle = HEX(C.muted); g.font = '400 28px ' + MONO; g.fillText('коментар · src/screens/dashboard/Dashboard.tsx : 42', 136, 100);
      pill(g, W - 44 - (g.font = '600 28px ' + MONO, g.measureText(resolved ? 'Resolved ✓' : 'Open').width + 44), 46, resolved ? 'Resolved ✓' : 'Open', col, resolved ? 'rgba(46,160,67,.18)' : 'rgba(219,109,40,.18)', '600 28px ' + MONO);
      // рядок коду
      g.fillStyle = '#010409'; rr(g, 44, 150, W - 88, 84, 14); g.fill();
      g.fillStyle = HEX(C.faint); g.font = '400 28px ' + MONO; g.fillText('42', 68, 192);
      g.fillStyle = HEX(C.text); g.font = '400 32px ' + MONO; g.fillText('<Button variant="secondary" size="sm">Експорт CSV</Button>', 130, 192);
      // текст
      g.fillStyle = HEX(C.text); g.font = '400 36px ' + SANS;
      wrap(g, 'Кнопка Експорт має бути disabled, поки дані вантажаться — інакше можна клікнути двічі й отримати два файли.', 44, 292, W - 88, 50);
      if (resolved) {
        g.fillStyle = 'rgba(46,160,67,.12)'; rr(g, 44, 470, W - 88, 210, 16); g.fill();
        g.fillStyle = HEX(C.blue); g.beginPath(); g.arc(90, 522, 26, 0, Math.PI * 2); g.fill();
        g.fillStyle = HEX(C.bg); g.font = '700 26px ' + SANS; g.textAlign = 'center'; g.fillText('А', 90, 524); g.textAlign = 'left';
        g.fillStyle = HEX(C.text); g.font = '600 32px ' + SANS; g.fillText('Аня', 134, 522);
        g.fillStyle = HEX(C.muted); g.font = '400 30px ' + SANS; wrap(g, 'Додала стан disabled і спінер. Коміт 8be04d1 у цій же гілці.', 134, 580, W - 200, 42);
      } else {
    ...
    function viewCard(who) {
      const W = 1280, H = 800, el = cv(W, H), g = el.getContext('2d');
      const author = who === 'author'; const col = author ? HEX(C.blue) : HEX(C.orange);
      g.fillStyle = HEX(C.surface); rr(g, 8, 8, W - 16, H - 16, 26); g.fill();
      g.strokeStyle = col; g.lineWidth = 6; rr(g, 8, 8, W - 16, H - 16, 26); g.stroke();
      g.textBaseline = 'middle';
    ...
    const work = mk.tube([[X.base, Y_M, 0], [-5.5, Y_M + 0.9, 0], [-5.0, Y_B, 0], [-2, Y_B, 0], [X.tip, Y_B, 0]], C.blue);
    const mergeT = mk.tube([[X.tip, Y_B, 0], [5.5, Y_B, 0], [5.9, Y_M + 0.9, 0], [X.merge, Y_M, 0]], C.blue);
    ...
    const lineToCode = mk.leader(0, 0, 0, C.orange); // замінимо геометрію під час кадру
    const viewA = mk.card(viewCard('author'), -2.6, 2.2, 4.6), viewB = mk.card(viewCard('reviewer'), 3.2, 2.2, 4.6);
    ...
    ep.update = function (t) {
      ...
      lineToCode.material.color.setHex(t >= 55.0 ? C.green : C.orange);
